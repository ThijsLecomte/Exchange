<#
.SYNOPSIS
    This script changes the UPN Suffix for all users you select.
    Users can be selected from a CSV, specific OU or all AD users

.DESCRIPTION
   This script will prompt the administrator to select the import method the wishes to use:
        - All AD users
        - Import from CSV
        - All users from OU

   If the admin chooses CSV, he will be prompted to select a CSV file.
        The CSV should have 'Name' as a header

   If he chooses the OU option, he will need to select the OU he whiches to use.

   For these users it will change the UPN Suffix.
   The script will get all possible suffixes and change it to the desired one.

   Credits for the OU picker go to ITMicah
...
.PARAMETER LogPath
    This defines the path of the logfile. By default: "C:\Windows\Temp\CustomScript\Set-UPN.txt"
...
.EXAMPLE
    ...
    ..\Set-UPN.ps1 -logPath "C:\Windows\Temp\CustomScripts\Set-UPN.txt"

.NOTES
    File Name  : Set-UPN.ps1  
...
param (
    $LogPath = "C:\Windows\Temp\CustomScripts\ConnectTo-ExchangeOnline.txt"
)
#endregion
...
#Define Log function
Function Write-Log {
    Param ([string]$logstring,$ForeGroundColor)
...
Function Choose-Suffix($suffixes){
    Write-Log "[INFO] - Starting Function Choose-Suffix"
    Add-Type -AssemblyName System.Windows.Forms
    Add-Type -AssemblyName System.Drawing

    $form = New-Object System.Windows.Forms.Form
    $form.Text = 'Please select a UPN Suffix:'
    ...
    $label.Text = 'Please select a UPN Suffix:'
    ...
    {
        $selectedSuffix = $listBox.SelectedItem
        Write-Log "[INFO] - $selectedSuffix has been selected"
    }
    else{
        Write-Log "[ERROR] - No suffix has been selected, exiting" -ForeGroundColor Red
        Break
    }
    ...
    $MessageBody = "Are you sure you want to select the suffix $selectedSuffix"
    $MessageTitle = "Confirm suffix"

    $Result = [System.Windows.MessageBox]::Show($MessageBody,$MessageTitle,$ButtonType,$MessageIcon)

    if($result -eq "No"){
        Choose-Suffix -suffixes $suffixes
        Write-Log "[INFO] - User requested to reselect suffix"
    }

    Write-Log "[INFO] - Ending Function Choose-Suffix"

    return $selectedSuffix
}

Function Select-Method{
    Write-Log "[INFO] - Starting Function Select-Method"
    Add-Type -AssemblyName System.Windows.Forms
    Add-Type -AssemblyName System.Drawing

    $form = New-Object System.Windows.Forms.Form
    $form.Text = 'Select a method'
    ...
    $label.Location = New-Object System.Drawing.Point(10,20)
    $label.Size = New-Object System.Drawing.Size(280,20)
    $label.Text = 'Please select a method to import the users:'
    $form.Controls.Add($label)
    ...
    [void] $listBox.Items.Add('OU')
    [void] $listBox.Items.Add('CSV')
    ...
    switch($listBox.SelectedItem){
        "OU" {$users = Get-UsersFromOU;break}
        "CSV" {$users = BrowseandImport-Csv;break}
        "AD" {$users = Get-AllUsersFromAD;break}
    }

    Write-Log "[INFO] - Ending Function Select-Method"

    return $users
}

Function BrowseandImport-Csv{
    Write-Log "[INFO] - Starting Function BrowseandImport-Csv"
    ...
    Try{
        ...
        Write-Log "[INFO] - Got $($users.count) users"
    }
    Catch{
        Write-Log "[ERROR] - Getting users from CSV"
        Write-Log "$($_.Exception.Message)"
    }

    
    Write-Log "[INFO] - Ending Function BrowseandImport-Csv"

    return $users
}

Function Get-AllUsersFromAD{
    Write-Log "[INFO] - Starting Function Get-AllUsersFromAD"
    ...
    Catch{
        Write-Log "[ERROR] - Getting users from AD"
        Write-Log "$($_.Exception.Message)"
    }

    Write-Log "[INFO] - Got $($users.count) users"

    Write-Log "[INFO] - Ending Function Get-AllUsersFromAD"
    return $users
}

Function Get-UsersFromOU{
    Write-Log "[INFO] - Starting Function Get-UsersFromOU"
    $ChosenOU = Choose-ADOrganizationalUnit

    Write-Log "[INFO] - The chosen OU is: $($ChosenOU.DistinguishedName)"

    Try{
        $users = Get-AdUser -Filter * -SearchBase $chosenOU
        Write-Log "[INFO] - Got $($users.count) users"
    }
    Catch{
        Write-Log "[ERROR] - Getting users from OU"
        Write-Log "$($_.Exception.Message)"
    }

    Write-Log "[INFO] - Ending Function Get-UsersFromOU"
    return $users
}
...
$suffixes = (Get-ADForest | Select-Object -Property UPNSuffixes).UPNSuffixes

$suffix = Choose-Suffix -suffixes $suffixes


foreach($user in $users){
    Try{
        $userObject = Get-ADUser -Filter "Name -eq '$($user.Name)'" 
    }
    Catch{
        Write-Log "[ERROR] - Error getting userobject for $($user.Name)"
        Write-Log "$($_.Exception.Message)"
    }
    
    $newUPN = $userObject.UserPrincipalName -replace "\@(.*)","@$suffix"
    Write-Log "Setting UPN for $($user.Name) from $($userObject.UserPrincipalName) to $newUPN"

    try{
        Set-ADUser -Identity $userObject -UserPrincipalName $newUPN
    }
    catch{
        Write-Log "[ERROR] - Setting UPN for users"
        Write-Log "$($_.Exception.Message)"
    }
}